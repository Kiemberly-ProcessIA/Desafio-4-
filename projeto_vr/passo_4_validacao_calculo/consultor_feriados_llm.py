...
class ConsultorFeriadosLLM:
    ...
    def __init__(self, api_key: str = None, model: str = None):
        ...
        if genai is not None:
            ...
            logger.warning(
                "Pacote google-generativeai não encontrado. Usando modo fallback."
            )
            self.model = None
    ...
    def _consultar_gemini(self, prompt: str) -> Dict:
        ...
        try:
            ...
            if response and response.text:
                ...
                logger.warning(
                    "Gemini não retornou JSON válido, processando resposta texto"
                )
    ...
    def _obter_feriados_municipais_conhecidos(self, estado: str) -> List[Dict]:
        ...
        feriados_municipais = {
            ...
            "Rio de Janeiro": [
                {
                    "data": "2025-03-01",
                    "nome": "Aniversário do Rio de Janeiro",
                    "tipo": "municipal",
                }
            ],
            "Rio Grande do Sul": [
                {
                    "data": "2025-03-26",
                    "nome": "Aniversário de Porto Alegre",
                    "tipo": "municipal",
                }
            ],
            "Paraná": [
    ...
    def _calcular_dias_uteis_periodo(
        self, data_inicio: date, data_fim: date, feriados: List[Dict]
    ) -> Dict:
            ...
            if data_atual.weekday() < 5:  # 0-4 = segunda a sexta
                if data_atual not in datas_feriados:
                    ...
                    for feriado in feriados:
                        try:
                            if (
                                datetime.strptime(feriado["data"], "%Y-%m-%d").date()
                                == data_atual
                            ):
    ...
    def _obter_valor_vr_conhecido(self, estado: str) -> Dict:
        ...
        valores_conhecidos = {
            "São Paulo": {
                "valor": 37.5,
                "moeda": "BRL",
                "base_legal": "Convenção Coletiva SINDPD-SP",
                "vigencia": "2025-01-01",
            },
            "Rio de Janeiro": {
                "valor": 35.0,
                "moeda": "BRL",
                "base_legal": "Convenção Coletiva SINDPD-RJ",
                "vigencia": "2025-01-01",
            },
            "Rio Grande do Sul": {
                "valor": 35.0,
                "moeda": "BRL",
                "base_legal": "Convenção Coletiva SINDPPD-RS",
                "vigencia": "2025-01-01",
            },
            "Paraná": {
                "valor": 35.0,
                "moeda": "BRL",
                "base_legal": "Convenção Coletiva SITEPD-PR",
                "vigencia": "2025-01-01",
            },
        }
        return valores_conhecidos.get(
            estado,
            {
                "valor": 35.0,
                "moeda": "BRL",
                "base_legal": "Padrão Nacional",
                "vigencia": "2025-01-01",
            },
        )
    ...
    def _obter_feriados_padrao(self, estado: str, ano: int) -> Dict:
        ...
        dias_uteis_info = self._calcular_dias_uteis_periodo(
            date(2025, 4, 15), date(2025, 5, 15), todos_feriados
        )
...
def criar_consultor_feriados(
    api_key: str = None, model: str = None
) -> ConsultorFeriadosLLM:
    """
...
if __name__ == "__main__":
    ...
    for estado in estados_teste:
        ...
        try:
            ...
            print(
                f"  Dias úteis (15/04 a 15/05): {info['dias_uteis_periodo']['total_dias_uteis']}"
            )
