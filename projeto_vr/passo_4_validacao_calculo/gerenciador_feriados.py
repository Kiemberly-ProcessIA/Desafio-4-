...
class GerenciadorFeriados:
    ...
    def _obter_informacoes_fallback(self, estado: str) -> Dict:
        ...
        feriados_nacionais = [
            {
                "data": f.strftime("%Y-%m-%d"),
                "nome": self._obter_nome_feriado(f),
                "tipo": "nacional",
            }
        ...
        feriados_municipais = [
            ...
            for f in self.feriados_municipais.get(
                self._obter_capital_estado(estado), set()
            )
        ]
        ...
        dias_uteis = self._calcular_dias_uteis_com_feriados(
            periodo_inicio, periodo_fim, todos_feriados
        )
    ...
    def _calcular_dias_uteis_com_feriados(
        self, data_inicio: date, data_fim: date, feriados: Set[date]
    ) -> int:
            ...
            try:
                ...
                for categoria in [
                    ...
                    for feriado in info.get(categoria, []):
                        try:
                            data_feriado = datetime.strptime(
                                feriado["data"], "%Y-%m-%d"
                            ).date()
    ...
    def calcular_dias_uteis(
        self, data_inicio: date, data_fim: date, estado: str, municipio: str = None
    ) -> int:
        """
        ...
        if self.usar_llm:
            try:
                ...
                if periodo_info.get("inicio") == data_inicio.strftime(
                    "%Y-%m-%d"
                ) and periodo_info.get("fim") == data_fim.strftime("%Y-%m-%d"):
                    ...
                    return self._calcular_dias_uteis_com_feriados(
                        data_inicio, data_fim, feriados
                    )
            except Exception as e:
        ...
        for feriado in sorted(feriados):
            if data_inicio <= feriado <= data_fim:
                ...
                if feriado in self.feriados_estaduais.get(estado, set()):
                    tipo = "Estadual"
                elif municipio and feriado in self.feriados_municipais.get(
                    municipio, set()
                ):
                    tipo = "Municipal"
    ...
    def gerar_relatorio_feriados(
        self, estado: str, municipio: str = None, arquivo_saida: str = None
    ) -> str:
        """
        ...
        feriados_periodo = self.listar_feriados_periodo(
            periodo_inicio, periodo_fim, estado, municipio
        )
        dias_uteis = self.calcular_dias_uteis(
            periodo_inicio, periodo_fim, estado, municipio
        )
        ...
        if feriados_periodo:
            ...
            for feriado in feriados_periodo:
                relatorio.append(
                    f"{feriado['data'].strftime('%d/%m/%Y')} - {feriado['nome']} ({feriado['tipo']})"
                )
        else:
...
def obter_dias_uteis_estado(
    estado: str, data_inicio: date = None, data_fim: date = None
) -> int:
    """
...
def salvar_cache_feriados(arquivo: str = "cache_feriados.json"):
    ...
    try:
        with open(arquivo, "w", encoding="utf-8") as f:
            json.dump(
                gerenciador_feriados.cache_informacoes,
                f,
                indent=2,
                ensure_ascii=False,
                default=str,
    ...
    for estado in estados:
        ...
        try:
            ...
            if feriados_periodo:
                ...
                for feriado in feriados_periodo:
                    print(
                        f"    - {feriado['data']}: {feriado['nome']} ({feriado['tipo']})"
                    )
            else:
            ...
            # Testar fallback
            try:
                dias_uteis_fallback = gerenciador_feriados.calcular_dias_uteis(
                    periodo_inicio, periodo_fim, estado
                )
