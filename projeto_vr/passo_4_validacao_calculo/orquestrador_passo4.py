...
class OrquestradorPasso4:
    ...
    def _executar_validacao(self, arquivo_entrada: str) -> Dict[str, Any]:
        ...
        self.validador.gerar_relatorio_validacao(f"{self.output_path}/passo_4-relatorio_validacao.txt")
    ...
    def _executar_calculo(self, dados_validados: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Gerar relatórios
        self.calculador.gerar_relatorio_calculo(f"{self.output_path}/passo_4-relatorio_calculo_vr.txt")
    ...
    def _gerar_relatorio_consolidado(self, dados_finais: Dict[str, Any]):
        ...
        colaboradores_elegíveis = sum(1 for c in colaboradores.values() 
                                    if c.get('calculo_vr', {}).get('elegivel', False))
        valor_total_vr = sum(c.get('calculo_vr', {}).get('valor_total', 0.0) 
        ...
        for colaborador in colaboradores.values():
            ...
            if calculo_vr.get('elegivel', False):
                ...
                estatisticas_estado[estado]['valor_total'] += calculo_vr.get('valor_total', 0.0)
                estatisticas_estado[estado]['dias_uteis_medio'] += calculo_vr.get('dias_uteis', 0)
        
        # Calcular médias
        for estado_data in estatisticas_estado.values():
            if estado_data['colaboradores'] > 0:
                estado_data['dias_uteis_medio'] = estado_data['dias_uteis_medio'] / estado_data['colaboradores']
        ...
        with open(arquivo_relatorio, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Taxa de elegibilidade: {(colaboradores_elegíveis/total_colaboradores*100):.1f}%\n")
            f.write(f"Valor total de VR: R$ {valor_total_vr:,.2f}\n")
            f.write(f"Valor médio por colaborador elegível: R$ {(valor_total_vr/colaboradores_elegíveis if colaboradores_elegíveis > 0 else 0):,.2f}\n\n")
            
            # Estatísticas de validação
            f.write("ESTATÍSTICAS DE VALIDAÇÃO\n")
            f.write("-" * 30 + "\n")
            f.write(f"Colaboradores com dados corrigidos: {metadata_validacao.get('colaboradores_corrigidos', 0)}\n")
            f.write(f"Total de problemas encontrados: {len(metadata_validacao.get('relatorio', []))}\n\n")
            ...
            for estado, stats in estatisticas_estado.items():
                ...
                f.write(f"  Valor médio por colaborador: R$ {(stats['valor_total']/stats['colaboradores']):,.2f}\n\n")
            ...
            for tipo, count in sorted(tipos_correcao.items(), key=lambda x: x[1], reverse=True):
    ...
    def gerar_resumo_executivo(self, dados_finais: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for colaborador in colaboradores.values():
            ...
            if calculo_vr.get('elegivel', False):
                ...
                resumo['distribuicao_estados'][estado]['valor'] += calculo_vr.get('valor_total', 0.0)
...
def executar_passo4(arquivo_entrada: str = None, config_path: str = None, output_path: str = None) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--entrada', '-e', help='Arquivo de entrada (JSON)', 
