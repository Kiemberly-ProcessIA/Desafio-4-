...
class CalculadorBeneficios:
    ...
    def _carregar_dados_sindicatos(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.info(f"Dados de sindicatos carregados do arquivo: {len(dados)} sindicatos")
            return dados
            ...
            from .gerenciador_feriados import gerenciador_feriados, obter_dias_uteis_estado
            ...
            return dados_llm if dados_llm else {
        ...
        try:
            ...
            for estado in estados:
                try:
                    ...
                    logger.error(f"Erro ao consultar valor VR via LLM para {estado}: {e_llm}")
                    # Fallback para valores conhecidos
                    valores_fallback = {
                        'Paraná': 35.0,
                        'Rio de Janeiro': 35.0,
    ...
    def _identificar_estado_sindicato(self, sindicato: str) -> str:
        ...
        if any(x in sindicato_lower for x in ['sp', 'são paulo', 'sindpd', 'sindpd-sp']):
            return 'São Paulo'
        elif any(x in sindicato_lower for x in ['rs', 'rio grande do sul', 'gaúcho', 'gaucho', 'sindppd']):
            return 'Rio Grande do Sul'
        elif any(x in sindicato_lower for x in ['rj', 'rio de janeiro', 'carioca']):
            return 'Rio de Janeiro'
        elif any(x in sindicato_lower for x in ['pr', 'paraná', 'parana', 'curitiba', 'sitepd']):
            return 'Paraná'
        else:
    ...
    def calcular_dias_uteis_colaborador(self, colaborador: Dict[str, Any]) -> int:
        ...
        if demissao:
            ...
            if dias_proporcional < dias_uteis_base:
                ...
                motivos_desconto.append(f"Demissão proporcional: {dias_proporcional} dias")
    ...
    def processar_base_completa(self, dados_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for matricula, colaborador in colaboradores.items():
            ...
            estado = self._identificar_estado_sindicato(colaborador.get('sindicato', ''))
        ...
        logger.info(f"Cálculo concluído. {colaboradores_elegíveis}/{total_colaboradores} colaboradores elegíveis")
    ...
    def gerar_planilha_pagamento(self, dados_json: Dict[str, Any], arquivo_saida: str = "planilha_pagamento_vr.json"):
        ...
        with open(arquivo_saida, 'w', encoding='utf-8') as f:
            json.dump({
                'metadata': {
                    ...
                    'valor_total': sum(item['valor_total'] for item in planilha_pagamento)
                },
...
def executar_calculo_vr(arquivo_entrada: str, arquivo_saida: str = None) -> Dict[str, Any]:
    """
    ...
    calculador.gerar_planilha_pagamento(dados_calculados, "output/planilha_pagamento_vr.json")
...
if __name__ == "__main__":
    ...
    print(f"Cálculo concluído. {len(dados_calculados.get('colaboradores', {}))} colaboradores processados.")
