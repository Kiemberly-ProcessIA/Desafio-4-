...
class GeradorPlanilhaFinal:
    ...
    def gerar_planilha_operadora(self, dados_validados: Dict[str, Any]) -> Dict[str, str]:
        ...
        temp_json = os.path.join(tempfile.gettempdir(), f"vr_dados_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(temp_json, 'w', encoding='utf-8') as f:
            json.dump({
                'colaboradores': dados_validados.get('colaboradores', {}),
                'metadata': dados_validados.get('metadata', {}),
                'estatisticas': {
                    'total_colaboradores': len(dados_validados.get('colaboradores', {})),
        ...
        temp_relatorio = os.path.join(tempfile.gettempdir(), f"vr_relatorio_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    def _gerar_csv_operadora(self, dados: Dict[str, Any]) -> str:
        ...
        nome_arquivo = f"VR_MENSAL_OPERADORA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        with open(caminho_arquivo, 'w', newline='', encoding='utf-8') as arquivo:
            writer = csv.writer(arquivo, delimiter=';')  # Usar ponto e vírgula como separador
    ...
    def _gerar_json_operadora(self, dados: Dict[str, Any]) -> str:
        ...
        nome_arquivo = f"VR_MENSAL_DADOS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        caminho_arquivo = self.diretorio_output / nome_arquivo
        ...
        dados_operadora['cabecalho']['valor_total_funcionario'] = float(total_funcionario)
        ...
        dados_operadora['resumo']['estatisticas_valores'] = {
            'valor_medio_vr': float(total_vr / len(dados.get('colaboradores', {}))) if dados.get('colaboradores') else 0,
            'maior_valor': max([float(c.get('valor_vr_calculado', 0)) for c in dados.get('colaboradores', {}).values()], default=0),
            'menor_valor': min([float(c.get('valor_vr_calculado', 0)) for c in dados.get('colaboradores', {}).values() if float(c.get('valor_vr_calculado', 0)) > 0], default=0)
        }
        
        # Salvar JSON
        with open(caminho_arquivo, 'w', encoding='utf-8') as arquivo:
            json.dump(dados_operadora, arquivo, indent=2, ensure_ascii=False, default=str)
        
        logger.info(f"JSON gerado com dados de {len(dados_operadora['colaboradores'])} colaboradores")
        return str(caminho_arquivo)
    ...
    def _gerar_relatorio_controle(self, dados: Dict[str, Any]) -> str:
        ...
        for colaborador in colaboradores.values():
            ...
            total_funcionario += valor_vr * Decimal(str(self.custo_funcionario_percentual))
        ...
        relatorio.append(f"Valor médio por colaborador: R$ {total_vr/total_colaboradores if total_colaboradores > 0 else 0:,.2f}")
        relatorio.append("")
        ...
        for estado, dados in sorted(distribuicao_estado.items()):
            relatorio.append(f"{estado}: {dados['count']} colaboradores - R$ {dados['valor']:,.2f}")
        relatorio.append("")
        
        relatorio.append("DISTRIBUIÇÃO POR SITUAÇÃO:")
        relatorio.append("-"*40)
        for situacao, dados in sorted(distribuicao_situacao.items()):
            relatorio.append(f"{situacao}: {dados['count']} colaboradores - R$ {dados['valor']:,.2f}")
        relatorio.append("")
        ...
        if validacao:
            ...
            relatorio.append("-"*40)
            relatorio.append(f"Registros processados: {estatisticas.get('total_registros', 0)}")
            relatorio.append(f"Registros válidos: {estatisticas.get('registros_validos', 0)}")
            relatorio.append(f"Registros com erro: {estatisticas.get('registros_com_erro', 0)}")
            relatorio.append(f"Registros com warning: {estatisticas.get('registros_com_warning', 0)}")
            
            if estatisticas.get('total_registros', 0) > 0:
                taxa_aprovacao = (estatisticas.get('registros_validos', 0) / estatisticas['total_registros']) * 100
    ...
    def _gerar_excel_operadora(self, dados: Dict[str, Any]) -> str:
        ...
        nome_arquivo = f"VR_MENSAL_OPERADORA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        caminho_arquivo = self.diretorio_output / nome_arquivo
        ...
        # Criar DataFrame
        colunas = [
            'matricula', 'admissao', 'sindicato', 'competencia', 'dias', 
            'valor diario', 'TOTAL', 'custo empresa', 'deconto funcionario', 'OBS GERAL'
        ]
    ...
    def _gerar_relatorio_temporario(self, dados: Dict[str, Any], caminho_arquivo: str):
        ...
        relatorio.append(f"• Desconto funcionário (20%): R$ {valor_funcionario_total:,.2f}")
        relatorio.append("")
