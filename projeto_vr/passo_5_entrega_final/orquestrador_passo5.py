...
class OrquestradorPasso5:
    ...
    def _carregar_dados_passo4(self, arquivo_entrada: Optional[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Dados carregados: {len(dados.get('colaboradores', {}))} colaboradores")
        
        # Preparar dados com campos necessários para operadora
        dados_preparados = self._preparar_dados_operadora(dados)
        
        self.resultados['estatisticas']['colaboradores_entrada'] = len(dados.get('colaboradores', {}))
        ...
        for matricula, colaborador in dados.get('colaboradores', {}).items():
            ...
            if 'calculo_vr' in colaborador_preparado:
                ...
                if 'valor_total' in calculo_vr:
                    colaborador_preparado['valor_vr_calculado'] = calculo_vr['valor_total']
    ...
    def _analisar_modelo_operadora(self):
        ...
        try:
            ...
            logger.info(f"Modelo analisado: {len(estrutura.get('colunas', []))} colunas identificadas")
    ...
    def _validar_para_operadora(self, dados: Dict[str, Any]) -> Dict[str, Any]:
        ...
        estatisticas_validacao = dados_validados.get('validacao', {}).get('estatisticas', {})
        logger.info(f"Validação concluída: {estatisticas_validacao.get('registros_validos', 0)} registros válidos")
        
        return dados_validados
    
    def _gerar_planilhas_finais(self, dados_validados: Dict[str, Any]) -> Dict[str, str]:
    ...
    def _finalizar_passo5(self, dados_validados: Dict[str, Any], arquivos_gerados: Dict[str, str]):
    ...
    def _limpar_outputs_antigos(self, excel_atual: str):
        ...
        try:
            ...
            for arquivo in glob.glob(str(self.diretorio_output / "VR_MENSAL_OPERADORA_*.xlsx")):
                if arquivo != excel_atual:
                    os.remove(arquivo)
                    logger.debug(f"Removido arquivo antigo: {os.path.basename(arquivo)}")
