...
class AuditorLLM:
    ...
    def executar_auditoria_completa(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validacao_exclusoes = self._validar_exclusoes_com_llm(regras_projeto, dados_finais)
            ...
            validacao_calculos = self._validar_calculos_com_llm(regras_projeto, dados_finais)
            ...
            self.resultados_auditoria.update({
                ...
                'aprovacao_final': score_final >= 0.85,  # 85% de conformidade mínima
                'relatorio_completo': relatorio_final
            })
            
            logger.info(f"🎉 === AUDITORIA CONCLUÍDA ===")
            logger.info(f"📊 Score de Conformidade: {score_final:.1%}")
            logger.info(f"✅ Aprovação: {'SIM' if self.resultados_auditoria['aprovacao_final'] else 'NÃO'}")
    ...
    def _validar_exclusoes_com_llm(self, regras: Dict[str, Any], dados: Dict[str, Any]) -> Dict[str, Any]:
        """Valida exclusões aplicadas usando LLM."""
        logger.info("🔍 Iniciando validação de exclusões com LLM")
        
        # Preparar dados para análise
        exclusoes_aplicadas = dados.get('passo_2-resultado_llm', {}).get('analise_llm', {})
        ...
        try:
            ...
            resultado_llm = json.loads(resposta.text.strip().replace('```json', '').replace('```', ''))
    ...
    def _validar_calculos_com_llm(self, regras: Dict[str, Any], dados: Dict[str, Any]) -> Dict[str, Any]:
        ...
        estatisticas_calculos = self._extrair_estatisticas_calculos(base_final, resumo_executivo)
        
        # Construir prompt para validação de cálculos
        prompt_calculos = self._construir_prompt_validacao_calculos(estatisticas_calculos)
    ...
    def _validar_legislacao_com_llm(self, dados: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt_legislacao = self._construir_prompt_validacao_legislacao(resumo_executivo)
    ...
    def _construir_prompt_validacao_exclusoes(self, exclusoes: Dict[str, Any], relatorio: str) -> str:
...
"""
    ...
    def _extrair_estatisticas_calculos(self, base_final: Dict[str, Any], resumo: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for estado_info in stats_por_estado.values():
            if estado_info['colaboradores'] > 0:
                estado_info['valor_medio'] = estado_info['valor_total'] / estado_info['colaboradores']
        
        return {
            'total_colaboradores': len(colaboradores),
            'valor_total_vr': float(total_vr),
            'valor_medio_geral': float(total_vr / len(colaboradores)) if colaboradores else 0,
            'distribuicao_por_estado': stats_por_estado,
            'resumo_executivo': resumo
        }
    
    def _calcular_score_conformidade(self, val_exclusoes: Dict, val_calculos: Dict, val_legislacao: Dict) -> float:
    ...
    def _gerar_relatorio_auditoria(self, val_exclusoes: Dict, val_calculos: Dict, val_legislacao: Dict) -> str:
        ...
        # Resumo dos scores
        score_final = self._calcular_score_conformidade(val_exclusoes, val_calculos, val_legislacao)
        ...
        relatorio.append(f"Status da Aprovação: {'✅ APROVADO' if aprovado else '❌ REPROVADO'}")
        ...
        relatorio.append(f"   Status: {'✅' if val_exclusoes.get('conformidade_exclusoes', False) else '❌'}")
        relatorio.append(f"   Justificativa: {val_exclusoes.get('justificativa', 'N/A')}")
        relatorio.append("")
        
        # Cálculos
        relatorio.append("2. VALIDAÇÃO DE CÁLCULOS:")
        relatorio.append(f"   Score: {val_calculos.get('score_calculos', 0):.1%}")
        relatorio.append(f"   Status: {'✅' if val_calculos.get('conformidade_calculos', False) else '❌'}")
        relatorio.append(f"   Justificativa: {val_calculos.get('justificativa', 'Validação baseada nas CCTs fornecidas')}")
        relatorio.append("")
        
        # Legislação
        relatorio.append("3. VALIDAÇÃO DE LEGISLAÇÃO:")
        relatorio.append(f"   Score: {val_legislacao.get('score_legislacao', 0):.1%}")
        relatorio.append(f"   Status: {'✅' if val_legislacao.get('conformidade_legislacao', False) else '❌'}")
        relatorio.append(f"   Justificativa: {val_legislacao.get('justificativa', 'Validação baseada na legislação trabalhista aplicável')}")
        relatorio.append("")
        ...
        if aprovado:
            ...
            relatorio.append("❌ Foram identificadas não-conformidades que devem ser corrigidas.")
...
def main():
    ...
    try:
        ...
        # Salvar resultado
        output_path = auditor.diretorio_output / f"passo_6-auditoria_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
