...
class AnalisadorExclusions:
    ...
    def __init__(self):
        ...
        if self.genai is not None:
            try:
                ...
                raise RuntimeError(f"❌ ERRO CRÍTICO: Não foi possível inicializar o cliente LLM: {e}")
        else:
            raise RuntimeError("❌ ERRO CRÍTICO: Pacote google.generativeai não encontrado. Instale com: pip install google-generativeai")
    ...
    def _carregar_ccts(self, caminho_convencoes: str) -> dict:
        ...
        try:
            ...
            if not caminho_convencoes.exists():
                logger.warning(f"⚠️ Pasta de convenções não encontrada: {caminho_convencoes}")
                return {}
            ...
            logger.info(f"✅ {len(ccts_info)} CCTs identificadas: {list(ccts_info.keys())}")
            return ccts_info
    ...
    def analisar_dados_passo1(self, caminho_colaboradores: str, caminho_configuracoes: str) -> dict:
        ...
        """
        log_inicio_passo("PASSO 2", "Análise LLM - Exclusões conforme Regras Oficiais (Diretores/Estagiários/Aprendizes)", logger)
    ...
    def analisar_com_dados_memoria(self, dados_consolidados: dict) -> dict:
        ...
        logger.info("💡 Dica: Para máxima eficiência, use o arquivo agrupamentos_consolidados.json")
    ...
    def analisar_com_agrupamentos_json(self, caminho_agrupamentos: str) -> dict:
        ...
        try:
            ...
            logger.info(f"🎯 {len(agrupamentos.get('cargos', []))} cargos únicos encontrados")
            logger.info(f"📊 {len(agrupamentos.get('status', []))} status únicos encontrados")
            logger.info(f"🔍 {len(agrupamentos.get('situacoes', []))} situações únicas encontradas")
    ...
    def _executar_analise_com_agrupamentos(self, agrupamentos: dict, ccts_info: dict = None) -> dict:
    ...
    def _executar_analise_com_dados(self, dados: dict) -> dict:
        ...
        try:
            ...
            estatisticas = {
                ...
                "situacoes_para_excluir": len(analise_llm.get('situacoes_para_excluir', [])),
            ...
            log_fim_passo("PASSO 2", f"Análise LLM concluída para {len(cargos_lista)} cargos", estatisticas, logger)
...
def main():
    ...
    caminho_colaboradores = Path(__file__).parent.parent.parent / "input_data" / "colaboradores"
    caminho_configuracoes = Path(__file__).parent.parent.parent / "input_data" / "configuracoes"
    ...
    resultado = analisador.analisar_dados_passo1(str(caminho_colaboradores), str(caminho_configuracoes))
    
    # Exibir resultado
    logger.info("📋 RESULTADO DA ANÁLISE:")
    logger.info(f"🎯 Total de cargos analisados: {resultado.get('dados_origem', {}).get('total_cargos_analisados', 'N/A')}")
    
    if 'analise_llm' in resultado:
        analise = resultado['analise_llm']
        logger.info(f"❌ Cargos para excluir: {len(analise.get('cargos_para_excluir', []))}")
        logger.info(f"✅ Cargos para manter: {len(analise.get('cargos_para_manter', []))}")
        
        # Mostrar alguns exemplos
        if analise.get('cargos_para_excluir'):
            logger.info("🔴 Exemplos de exclusões:")
            for cargo in analise['cargos_para_excluir'][:3]:
                logger.info(f"  • {cargo.get('cargo', 'N/A')}: {cargo.get('motivo', 'N/A')}")
