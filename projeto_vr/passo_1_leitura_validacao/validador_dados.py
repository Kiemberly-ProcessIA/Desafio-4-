...
class ValidadorDados:
    ...
    def validar_duplicatas_matricula(
        ...
        if not duplicatas.empty:
            ...
            for matricula in matriculas_dup:
                ...
                problemas.append(
                    f"{nome_arquivo}: Matrícula {matricula} aparece {count} vezes"
                )

        return problemas

    def validar_consistencia_matriculas(
        ...
        if (
            "desligados" in matriculas_por_arquivo
            and "ativos" in matriculas_por_arquivo
        ):
            ...
            if desligados_nao_encontrados:
                problemas.append(
                    f"Matrículas em DESLIGADOS mas não em ATIVOS: {desligados_nao_encontrados}"
                )
        ...
        if "ferias" in matriculas_por_arquivo and "ativos" in matriculas_por_arquivo:
            ...
            if ferias_nao_encontrados:
                problemas.append(
                    f"Matrículas em FÉRIAS mas não em ATIVOS: {ferias_nao_encontrados}"
                )

        return problemas

    def validar_sindicatos_consistencia(
        self, dados: Dict[str, pd.DataFrame]
    ) -> List[str]:
        ...
        if "ativos" in dados and "sindicato" in dados["ativos"].columns:
            ...
            if "dias_uteis" in dados and "sindicato" in dados["dias_uteis"].columns:
                sindicatos_dias = set(
                    dados["dias_uteis"]["sindicato"].dropna().unique()
                )
                ...
                if sem_dias_uteis:
                    problemas.append(
                        f"Sindicatos sem dias úteis definidos: {sem_dias_uteis}"
                    )
                ...
                if dias_sem_ativos:
                    problemas.append(
                        f"Sindicatos com dias úteis mas sem funcionários ativos: {dias_sem_ativos}"
                    )

        return problemas

    def validar_valores_numericos(
        self, df: pd.DataFrame, coluna: str, nome_arquivo: str
    ) -> List[str]:
        ...
        if not nao_numericos.empty:
            valores_invalidos = nao_numericos[coluna].unique()[
                :5
            ]  # Mostrar até 5 exemplos
            problemas.append(
                f"{nome_arquivo}: Valores não numéricos em '{coluna}': {valores_invalidos}"
            )
        ...
        if coluna in ["dias uteis", "valor", "dias de férias"]:
            ...
            if not negativos.empty:
                problemas.append(
                    f"{nome_arquivo}: Valores negativos encontrados em '{coluna}': {len(negativos)} registros"
                )
    ...
    def validar_datas(
        ...
        try:
            ...
            if not datas_invalidas.empty:
                problemas.append(
                    f"{nome_arquivo}: {len(datas_invalidas)} datas inválidas em '{coluna}'"
                )
            ...
            if "demissao" in coluna.lower():
                ...
                if not futuro.empty:
                    problemas.append(
                        f"{nome_arquivo}: {len(futuro)} datas de demissão no futuro"
                    )

        except Exception as e:
            problemas.append(
                f"{nome_arquivo}: Erro ao validar datas em '{coluna}': {e}"
            )

        return problemas

    def validar_completude_dados(
        self, df: pd.DataFrame, colunas_obrigatorias: List[str], nome_arquivo: str
    ) -> List[str]:
        ...
        for coluna in colunas_obrigatorias:
            if coluna not in df.columns:
                problemas.append(
                    f"{nome_arquivo}: Coluna obrigatória '{coluna}' ausente"
                )
                continue
            ...
            if total_problemas > 0:
                ...
                problemas.append(
                    f"{nome_arquivo}: '{coluna}' tem {total_problemas} valores vazios ({percentual:.1f}%)"
                )
    ...
    def executar_validacao_completa(
        ...
        if "ativos" in dados:
            ...
            problemas_ativos = self.validar_completude_dados(
                df_ativos, colunas_obrig, "ATIVOS"
            )
            todos_problemas.extend(problemas_ativos)

        if "valores" in dados:
            df_valores = dados["valores"]
            problemas_valores = self.validar_valores_numericos(
                df_valores, "valor", "BASE SINDICATO X VALOR"
            )
            todos_problemas.extend(problemas_valores)

        if "dias_uteis" in dados:
            df_dias = dados["dias_uteis"]
            problemas_dias = self.validar_valores_numericos(
                df_dias, "dias uteis", "BASE DIAS UTEIS"
            )
        ...
        if "desligados" in dados:
            ...
            if "demissao data" in df_desligados.columns:
                problemas_datas = self.validar_datas(
                    df_desligados, "demissao data", "DESLIGADOS"
                )
        ...
        resultado = {
            ...
            "problemas_criticos": [
                ...
                if any(
                    palavra in p.lower()
                    for palavra in ["ausente", "não encontrada", "duplicada"]
                )
            ],
            "avisos": [
                p
                ...
                if len(todos_problemas) == 0
                    ...
                    if len(
                        [
                            ...
                            if any(
                                palavra in p.lower()
                                for palavra in ["ausente", "não encontrada"]
                            )
                        ]
                    )
                    == 0
        ...
        if resultado["status_validacao"] == "APROVADO":
            ...
            logger.warning(
                f"⚠️ VALIDAÇÃO CONCLUÍDA: {len(resultado['avisos'])} avisos encontrados"
            )
        else:
            logger.error(
                f"❌ VALIDAÇÃO CONCLUÍDA: {len(resultado['problemas_criticos'])} problemas críticos encontrados"
            )
