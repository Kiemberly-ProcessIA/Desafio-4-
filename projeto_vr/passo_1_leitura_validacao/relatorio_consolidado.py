...
def gerar_relatorio_detalhado(caminho_json: str):
    ...
    print(f"✅ Ativos: {stats.get('ativos', 0):,} ({stats.get('ativos', 0)/stats.get('total_colaboradores', 1)*100:.1f}%)")
    print(f"🚪 Desligados: {stats.get('desligados', 0):,} ({stats.get('desligados', 0)/stats.get('total_colaboradores', 1)*100:.1f}%)")
    print(f"🆕 Admitidos no mês: {stats.get('admitidos_mes', 0):,} ({stats.get('admitidos_mes', 0)/stats.get('total_colaboradores', 1)*100:.1f}%)")
    print(f"🏖️  Em férias: {stats.get('em_ferias', 0):,} ({stats.get('em_ferias', 0)/stats.get('total_colaboradores', 1)*100:.1f}%)")
    ...
    print("-" * 60)
    for nome, info in sorted(sindicatos.items(), key=lambda x: x[1]['colaboradores_count'], reverse=True):
    ...
    print("💡 Para mais detalhes, consulte o arquivo JSON completo em ./output/base_consolidada.json")
    print("=" * 80)
