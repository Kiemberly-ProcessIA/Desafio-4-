...
from utils.logging_config import get_logger, log_resultado_validacao, log_processamento
...
class LeitorExcel:
    ...
    def ler_arquivo_excel(self, caminho: str, nome_arquivo: str) -> Optional[pd.DataFrame]:
        """Lê um arquivo Excel com tratamento de erros."""
        try:
            logger.info(f"📖 Carregando {nome_arquivo}...")
            df = pd.read_excel(caminho)
            logger.info(f"📊 {nome_arquivo}: {len(df)} linhas, {len(df.columns)} colunas")
            return df
            ...
            logger.error(f"❌ Erro ao processar {nome_arquivo}: {type(e).__name__}: {e}")
            return None
    ...
    def validar_estrutura_ativos(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validacao = {
            ...
            'colunas_ausentes': [col for col in colunas_esperadas if col not in colunas_presentes],
            'linhas_vazias': df.isnull().all(axis=1).sum(),
            'matriculas_duplicadas': df['matricula'].duplicated().sum() if 'matricula' in df.columns else 'N/A',
            'matriculas_nulas': df['matricula'].isnull().sum() if 'matricula' in df.columns else 'N/A',
            'situacoes_unicas': df['situacao'].unique().tolist() if 'situacao' in df.columns else 'N/A',
            'empresas_unicas': df['empresa'].unique().tolist() if 'empresa' in df.columns else 'N/A',
            'sindicatos_unicos': len(df['sindicato'].unique()) if 'sindicato' in df.columns else 'N/A'
        }
    ...
    def validar_estrutura_desligados(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validacao = {
            ...
            'comunicados_vazios': df['comunicado de desligamento'].isnull().sum() if 'comunicado de desligamento' in df.columns else 'N/A'
        }
    ...
    def validar_estrutura_ferias(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validacao = {
            ...
            'linhas_vazias': df.isnull().all(axis=1).sum(),
            'matriculas_duplicadas': df['matricula'].duplicated().sum() if 'matricula' in df.columns else 'N/A',
            'dias_ferias_media': df['dias de férias'].mean() if 'dias de férias' in df.columns else 'N/A',
            'dias_ferias_max': df['dias de férias'].max() if 'dias de férias' in df.columns else 'N/A',
            'situacoes_unicas': df['situacao'].unique().tolist() if 'situacao' in df.columns else 'N/A'
        }
    ...
    def validar_estrutura_dias_uteis(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validacao = {
            ...
            'colunas_presentes': colunas_presentes,
            'colunas_ausentes': [col for col in colunas_esperadas if col not in colunas_presentes],
            'sindicatos_total': len(df['sindicato'].unique()) if 'sindicato' in df.columns else 'N/A',
            'dias_uteis_min': df['dias uteis'].min() if 'dias uteis' in df.columns else 'N/A',
            'dias_uteis_max': df['dias uteis'].max() if 'dias uteis' in df.columns else 'N/A',
            'dias_uteis_media': df['dias uteis'].mean() if 'dias uteis' in df.columns else 'N/A'
        }
    ...
    def validar_estrutura_valores(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        validacao = {
            ...
            'colunas_presentes': colunas_presentes,
            'colunas_ausentes': [col for col in colunas_esperadas if col not in colunas_presentes],
            'estados_total': len(df['estado'].unique()) if 'estado' in df.columns else 'N/A',
    ...
    def executar_leitura_completa(self) -> Dict[str, any]:
        ...
        if status_arquivos.get('DESLIGADOS.xlsx', False):
            ...
            if df is not None:
                ...
                self.relatorio_validacao['desligados'] = self.validar_estrutura_desligados(df)
        ...
        if status_arquivos.get('Base dias uteis.xlsx', False):
            ...
            if df is not None:
                ...
                self.relatorio_validacao['dias_uteis'] = self.validar_estrutura_dias_uteis(df)
        
        # Base sindicato x valor.xlsx
        if status_arquivos.get('Base sindicato x valor.xlsx', False):
            caminho = os.path.join(self.pasta_configuracoes, 'Base sindicato x valor.xlsx')
        ...
        for arquivo in self.arquivos_opcionais:
            if status_arquivos.get(arquivo, False):
                ...
                if df is not None:
                    nome_chave = arquivo.replace('.xlsx', '').lower().replace(' ', '_').replace('ã', 'a')
    ...
    def gerar_resumo_validacao(self) -> Dict[str, any]:
        ...
        total_arquivos = len(self.arquivos_obrigatorios['colaboradores']) + len(self.arquivos_obrigatorios['configuracoes'])
        ...
        resumo = {
            ...
            'arquivos_opcionais_carregados': len([k for k in self.dados_carregados.keys() if k not in ['ativos', 'desligados', 'ferias', 'dias_uteis', 'valores']]),
            'status_geral': 'SUCESSO' if arquivos_carregados >= total_arquivos else 'PARCIAL',
    ...
    def imprimir_relatorio(self):
        ...
        for nome, validacao in self.relatorio_validacao.items():
            ...
            if validacao.get('linhas_vazias', 0) > 0:
                print(f"   ⚠️  Linhas completamente vazias: {validacao['linhas_vazias']}")
...
if __name__ == "__main__":
    ...
    print(f"📊 Arquivos carregados: {resumo['arquivos_carregados']}/{resumo['total_arquivos_obrigatorios']}")
